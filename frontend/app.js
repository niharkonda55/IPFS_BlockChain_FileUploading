...
async function loadABI() {
  try {
    ...
    alert("Failed to load contract ABI. Please check 'abi.json' file.");
  }
}
    ...
    try {
      ...
      window.ethereum.on('accountsChanged', (newAccounts) => {
        if (newAccounts.length === 0) {
          ...
          document.getElementById("recipientSelect").innerHTML = '<option value="">-- Select Wallet --</option>';
      ...
      window.ethereum.on('chainChanged', (chainId) => {
        ...
        // Prompt user to reload or automatically reload
        alert("Network changed. Please confirm the correct network in MetaMask and reload the page.");
        window.location.reload(); // Simple way to ensure Web3 context is correct
      });
      ...
      try {
        ...
        // Add new options, filtering out the current connected account
        allAccounts
        ...
        if (allAccounts.length === 0) {
          alert("No Ganache accounts found. Check Ganache setup.");
        } else if (select.options.length === 1) { // Only the default "--Select--" option remaining
          alert("No other Ganache accounts available to share with. Add more accounts in Ganache or connect a different one.");
        }
      } catch (error) {
        ...
        document.getElementById("recipientSelect").innerHTML = '<option value="">-- Ganache not connected --</option>';
      }

      // Immediately fetch received files after successful init
      getReceivedFiles();
      ...
      alert("MetaMask connection denied or failed. Please allow access.");
    }
...
// Attach init to window load event
window.addEventListener("load", init);
...
async function uploadFile() {
  ...
  if (!contract || !currentAccount) {
    alert("Wallet not connected or contract not initialized. Please refresh.");
    return;
  }
  ...
  try {
    const res = await fetch("https://api.pinata.cloud/pinning/pinFileToIPFS", {
      ...
      body: formData,
      headers: {
        'Authorization': pinataJWT // Correct JWT usage
      }
    });
    ...
    // Share the file hash on the blockchain
    console.log("Sharing file on blockchain...");
    const tx = await contract.methods.shareFile(recipient, ipfsHash).send({ from: currentAccount });
    console.log("Transaction complete:", tx);
    alert("File shared successfully!");
    ...
    // Refresh received files in case current account is also a recipient
    getReceivedFiles();

  } catch (err) {
    console.error("Upload/share failed:", err);
    // More descriptive error message for user
    alert(`Something went wrong during upload or blockchain transaction. Please check console for details.\nError: ${err.message || err}`);
  } finally {
...
async function getReceivedFiles() {
  ...
  if (!contract || !currentAccount) {
    // If wallet not connected or contract not loaded, display appropriate message
  ...
  try {
    ...
    files.forEach(entry => {
      ...
      // Create the wrapper div for "From" and link, matching HTML/CSS structure
      const fileInfoDiv = document.createElement('div');
      fileInfoDiv.classList.add('file-info'); // This class applies flex-direction: column and gap

      const fromSpan = document.createElement('span');
      // Shorten the uploader address for display (first 6, last 4 chars)
      fromSpan.textContent = `From: ${entry.uploader.substring(0, 6)}...${entry.uploader.slice(-4)}`;

      const linkAnchor = document.createElement("a");
      linkAnchor.href = url;
      linkAnchor.target = "_blank"; // Opens in a new tab
    ...
    alert("Failed to fetch received files. Check console for more details.");
  }
}